...
epsilon, xi, name, name_presaved, target_irrep, Ne, Ls = [x for x in sys.argv[5:]]
xi = float(xi)
...
path_to_presaved = sys.argv[4] + '/' + name_presaved + '/e_{:.2f}_xi_{:.2f}_Ne_{:d}/irrep_{:s}/'.format(epsilon, xi, Ne, target_irrep)
...
for i, line in enumerate(lines):
    ...
    if 'self.load_parameters_path = ' in line:        
        lines[i] = '        self.load_parameters_path = \'{:s}\'\n'.format(path_to_presaved)
