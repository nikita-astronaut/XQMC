...
U, V, fugacity = [float(x) for x in sys.argv[5:]]
...
config_name = os.path.join(path_to_configs, 'config_U_{:.2f}_V_{:.2f}_f_{:.2f}.py'.format(U, V, fugacity))
...
for i, line in enumerate(lines):
    if '#SBATCH -o' in line:
        lines[i] = '#SBATCH -o {:s}/output_U_{:.2f}_V_{:.2f}_f_{:.2f}.out\n'.format(U, V, fugacity)
    if '#SBATCH -e' in line:
        lines[i] = '#SBATCH -e {:s}/error_U_{:.2f}_V_{:.2f}_f_{:.2f}.err\n'.format(U, V, fugacity)
    if '#SBATCH --job-name' in line:
        lines[i] = '#SBATCH --job-name U_{:.2f}_V_{:.2f}_f_{:.2f}\n'.format(U, V, fugacity)
