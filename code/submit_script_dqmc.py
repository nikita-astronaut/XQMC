...
for n_copy in range(Nc):
    ...
    for i, line in enumerate(lines):
        ...
        if 'self.n_copy = ' in line:
            lines[i] = '        self.n_copy = {:d}\n'.format(n_copy)

    config_name = list(os.path.join(path_to_configs, 'config_U_{:.2f}_V_{:.2f}_mu_{:.2f}_Nt_{:d}_c_{:d}.py'.format(U, V, mu, Nt, n_copy)))
    ...
    for i, line in enumerate(lines):
        if '#SBATCH -o' in line:
            lines[i] = '#SBATCH -o {:s}/output_U_{:.2f}_V_{:.2f}_mu_{:.2f}_Nt_{:d}_c_{:d}.out\n'.format(path_to_logs, U, V, mu, Nt, n_copy)
        if '#SBATCH -e' in line:
            lines[i] = '#SBATCH -e {:s}/error_U_{:.2f}_V_{:.2f}_mu_{:.2f}_Nt_{:d}_c_{:d}.err\n'.format(path_to_logs, U, V, mu, Nt, n_copy)
        if '#SBATCH --job-name' in line:
            lines[i] = '#SBATCH --job-name U_{:.2f}_V_{:.2f}_mu_{:.2f}_Nt_{:d}_c_{:d}\n'.format(U, V, mu, Nt, n_copy)
